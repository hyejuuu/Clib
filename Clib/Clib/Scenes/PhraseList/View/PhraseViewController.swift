...
class PhraseViewController: UIViewController {
    ...
    @objc private func touchUpEtcButton() {
        ...
        let edtingAction = UIAlertAction(title: "문장 수정",
            ...
            let editingPhraseNavigator
                = UINavigationController(rootViewController: editingPhraseViewController)
            editingPhraseNavigator.modalPresentationStyle = .fullScreen
            self?.present(editingPhraseNavigator,
                          animated: true)
        }
    ...
    func tableView(
        ...
        guard let cell
            = tableView.dequeueReusableCell(withIdentifier: "PhraseCell")
                as? PhraseContentsTableViewCell else {
            return UITableViewCell()
        }
        
        cell.configure(page: phrase?.page,
